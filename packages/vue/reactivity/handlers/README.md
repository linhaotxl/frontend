...
我们看到，首先会获取 `indexOf` 方法，再获取数组的长度，然后从 `0` 开始，判断每个元素是否存在，存在的话就获取这个元素，与我们的传的参数比较，直至有相等的结果  
...
按照正常期望，`index` 应该是 `0`，现在结合 `get` 函数与上面 `indexOf` 的调用流程一起来看，获取 `indexOf` 方法、长度 `length` 已经判断第 `0` 个元素是否存在都没问题，现在到了获取第 `0` 个元素  

此时，获取到的是一个对象，而在 `get` 函数最后，会检测获取到的值是否是对象，所以，我们实际获取到的是一个经过 `reactive` 的响应对象，而不是原来的 `obj`，那么第 `1` 个元素也是如此，所以 `index` 实际是 `-1`  

`arrayInstrumentations` 的作用就是解决这个问题存在的，接下来具体看它的实现  

2. 首先看 ① 处的 `this`，这个 `this` 具体指向的是什么？还是基于上面的示例  
在 `get` 函数中，我们只是通过 `return Reflect.get( arrayInstrumentations, key, receiver )` 获得了具体的方法，而真正调用则是由 `arr` 发起的，所以 `this` 其实就是指向 `arr`  
