...
在创建 [vnode]() 时，通过 [normalizeSuspenseChildren](#normalizeSuspenseChildren) 会对这两个操作进行处理  

## Suspense 组件流程  
1. 每个 `Suspense` 组件都会有一个空的 `div` 容器，称为 `hiddenContainer`，首先会将 `default` `patch` 到 `hiddenContainer` 中  
    如果 `default` 中存在异步过程，会将异步组件的 `subTree` 设置为注释节点，然后将注释插入到 `hiddenContainer` 中，作为占位符  
...
## SuspenseImpl  
这是 `Suspense` 组件的具体实现，就是一个普通的对象，通过 `__isSuspense` 来标识这是一个 `Teleport` 组件，总共提供了三种操作函数   
...
function mountSuspense(
    ...
    // 3. 将 ssContent 挂载在 pendingBranch 上，表示这是 Suspense 需要等待的组件，并 patch ssContent 组件
    patch(
        null,
...
registerDep(instance, setupRenderEffect) {
    ...
    .then(asyncSetupResult => {
        ...
        setupRenderEffect(
            ...
            // placeholder.
            // 客户端下，占位符节点是处于 hiddenContainer 中的
            parentNode(hydratedEl || instance.subTree.el!)!,
            // anchor will not be used if this is hydration, so only need to
            // consider the comment placeholder case.
            // 客户端下，占位符节点的位置就是异步组件需要渲染的位置，所以需要渲染在占位符节点之前
